...
import com.example.track.seeta6.SeetaPointF
...
class DrawUtils() {
    fun adjustRect(
        ...
        Log.d(
            "dovt3: ", "adjustRect: " + previewWidth + " " + previewHeight + " " + canvasWidth +
    ...
    fun adjustPoint(
        ...
        val heightRatio = canvasHeight.toFloat() / previewHeight.toFloat()
        if (cameraOri == 0 || cameraOri == 180) {
            seetaPointF.x *= widthRatio.toInt()
            seetaPointF.y *= heightRatio.toInt()
        } else {
            var x = seetaPointF.x * heightRatio
            var y = seetaPointF.y * widthRatio

            seetaPointF.x = x
            seetaPointF.y = y
        }
        val newPoint = SeetaPointF()
        when (cameraOri) {
            0 -> {
                if (mCameraId == Camera.CameraInfo.CAMERA_FACING_FRONT) {
                    newPoint.x = canvasWidth - seetaPointF.x
                } else {
                    newPoint.x = seetaPointF.x
                }
                newPoint.y = seetaPointF.y
            }
            90 -> {
                newPoint.x = canvasWidth - seetaPointF.y
                if (mCameraId == Camera.CameraInfo.CAMERA_FACING_FRONT) {
                    newPoint.y = canvasHeight - seetaPointF.x
                } else {
                    newPoint.y = seetaPointF.x
                }
            }
            180 -> {
                newPoint.y = canvasHeight - seetaPointF.x
    ...
    fun drawSeetaPointF(canvas: Canvas?, seetaPointF: SeetaPointF?, color: Int, seetaPointFThickness: Int){
        if (canvas == null || seetaPointF == null) {
            return
        }
        val paint = Paint()
        paint.style = Paint.Style.STROKE
        paint.strokeWidth = seetaPointFThickness.toFloat()
        paint.color = color
        canvas.drawPoint(seetaPointF.x.toFloat(), seetaPointF.y.toFloat(), paint)
    }
}